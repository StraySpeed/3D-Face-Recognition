...
class BioHashFaceRecognizer:
    ...
    def register_gallery(self, data_root):
        ...
        for identity in identities:
            ...
            # 사람마다 시드가 달라지나 여기서는 인물 번호로 시드를 설정함
            person_seed = int(identity[-4:])
            ...
            if len(embeddings) > 0:
                ...
                bio_code = self.hasher.get_biocode(mean_embedding, person_seed)
                self.gallery_dict[identity] = bio_code
            else:
                print(f"Warning: No valid files found for {identity}")
                
        print(f"Total: {len(self.gallery_dict)} IDs.")

    def recognize(self, npy_path, seed, threshold=0.5):
        """
        새로운 얼굴(Probe) 인식
        """
        ...
        print("[1:N BioHashing Recognization Start]")
        ...
        # 추출 템플릿을 BioCode로 변환
        probe_code = self.hasher.get_biocode(probe_emb, seed)
        enc_time = time.time()
        print(f"2. Bio-Encryption Time: {enc_time - emb_time:.4f}")
        ...
        for gallery_id, gallery_code in self.gallery_dict.items():
            
            # 코사인 유사도 계산 (이미 정규화된 벡터이므로 내적과 동일)
            # Similarity = A . B (Range: -1 ~ 1)
            # Hamming Distance를 이용함
            similarity = self.hasher.hamming_distance(probe_code, gallery_code)
        ...
        print(f"[1:N BioHashing Matching Finished]")
    ...
    def recognize_id(self, npy_path, id, seed, threshold=0.5):
        """
        새로운 얼굴(Probe) 인식
        """
        if not os.path.exists(npy_path):
        ...
        print("[1:1 BioHashing Recognization Start]")
        start_time = time.time()
        input_tensor = self.preprocess(points)
        pre_time = time.time()
        print(f"0. Preprocess Time: {pre_time - start_time:.4f}")
        ...
        gallery_id = id
        gallery_code = self.gallery_dict.get(gallery_id, None)
        ...
        print(f"4. Total Matching Time: {end_time - start_time:.4f}")
        print(f"[1:1 BioHashing Matching Finished]")
    ...
    def save_gallery_individual(self, save_dir="./gallery_storage"):
        ...
        print(f"Saving individual biocode to '{save_dir}'...")
        
        count = 0
        for identity, biocode in self.gallery_dict.items():
            # 파일명: id_XXX.npy
            file_path = os.path.join(save_dir, f"{identity}.npy")
            np.save(file_path, biocode)
            count += 1
    ...
    def load_gallery_individual(self, load_dir="./gallery_storage"):
        ...
        for file_path in npy_files:
            ...
            # 로드
            biocode = np.load(file_path)
            self.gallery_dict[identity] = biocode
...
if __name__ == "__main__":
    ...
    for id in identities:
        ...
        for f in os.listdir(person_dir):
            ...
            person_seed = int(id[-4:])
            # 1:N Matching
            #identity, score = recognizer.recognize(identity_file, person_seed, threshold=0.3)

            # 1:1 Matching
            identity, score =recognizer.recognize_id(identity_file, id, person_seed, threshold=0.3)
    ...
    #recognizer.save_gallery_individual("./gallery_storage/umbdb_biocode")