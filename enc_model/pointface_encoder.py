...
class PointFaceEncoder(nn.Module):
    def __init__(self): # 보통 좌표(3) 또는 좌표+법선(6)인데 3을 이용
        ...
        # 설정 파일의 값을 그대로 가져오기
        self.sa1 = SetAbstraction(sa_params[0]['npoint'] // 4, sa_params[0]['radius'], sa_params[0]['nsample'], sa_params[0]['in_ch'], sa_params[0]['out_ch'], sa_params[0]['hid_ch'])
        self.sa2 = SetAbstraction(sa_params[1]['npoint'] // 2, sa_params[1]['radius'], sa_params[1]['nsample'], sa_params[1]['in_ch'], sa_params[1]['out_ch'], sa_params[1]['hid_ch'])
    ...
    def forward(self, x, pre_data=None):
        ...
        s1_points = CONFIG["SA_PARAMS"][0]["npoints"] // 4
        ...
        s2_points = CONFIG["SA_PARAMS"][1]["npoints"] // 2
        ...
        s3_points = CONFIG["SA_PARAMS"][2]["npoints"]
        ...
        s4_points = CONFIG["SA_PARAMS"][3]["npoints"]
