...
if __name__ == "__main__":
    ...
    for id in identities:
        ...
        for f in os.listdir(person_dir):
            ...
            probe_data = client.extract_and_encrypt(identity_file)
            
            # Server: 매칭 수행
            # 결과는 암호화된 (DotProduct, Norms) 리스트
            encrypted_results = server.match(probe_data, id)
            
            # Client: 결과 복호화 및 판단
            for res in encrypted_results:
                identity, enc_dot, enc_p_norm, enc_g_norm = res
                
                score = client.decrypt_result(enc_dot, enc_p_norm, enc_g_norm)
                if score < threshold:
                    identity = "Unknown"
