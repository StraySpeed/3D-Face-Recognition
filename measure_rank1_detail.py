...
# ──────────────────────────────────────────────────────────────
# 전처리 (measure_rank1.py 동일 로직)
# ──────────────────────────────────────────────────────────────

def _preprocess(points: np.ndarray, num_points: int, device: torch.device) -> torch.Tensor:
...
def _embed(net, npy_path: str, num_points: int, device: torch.device):
    ...
    try:
        ...
        tensor = _preprocess(points, num_points, device)
        with torch.no_grad():
...
def evaluate_umbdb(model_path: str, data_root: str,
                   num_classes: int, num_points: int, device: torch.device):
    print(f"\n{'='*58}")
    print(f"  UMBDB — 카테고리별 Rank-1")
    print(f"  데이터: {data_root}")
    print(f"{'='*58}")

    net = _load_model(model_path, num_classes, device)
...
def main():
    ...
    if args.dataset in ('umbdb', 'all'):
        ...
        evaluate_umbdb(ckpt, UMBDB_DATA, num_classes, num_points, device)

    if args.dataset in ('facescape', 'all'):
        ckpt = args.ckpt_facescape or _latest_checkpoint(FS_CKPT_DIR)
        evaluate_facescape(ckpt, FACESCAPE_DATA, num_classes, num_points, device)
