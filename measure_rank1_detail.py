...
모델 : model/pointface.py  (SABlock + Morton Z-order)
       SA 채널: 64 → 128 → 256 → 512, FC: 512 → feature_dim(256)
체크포인트: checkpoints6_umbdb / checkpoints6_facescape  (feature_dim=256)
...
사용법:
  python measure_rank1_detail.py
  python measure_rank1_detail.py --dataset umbdb
  python measure_rank1_detail.py --dataset facescape
  python measure_rank1_detail.py --ckpt_umbdb checkpoints6_umbdb/pointface_epoch_440.pth
"""
...
def _load_model(model_path: str, feature_dim: int, device: torch.device):
...
def _embed(net, npy_path: str, num_points: int, device: torch.device):
    if not os.path.exists(npy_path):
        return None
    try:
        points = np.load(npy_path)[:, :3]
        tensor = PointFaceNet.preprocess(points, num_points=num_points,
                                         device=str(device))
        with torch.no_grad():
...
def evaluate_umbdb(model_path: str, data_root: str,
    ...
    net = _load_model(model_path, feature_dim, device)
    ...
    for idx, ident in enumerate(identities):
        ...
        gal_emb = _embed(net, os.path.join(d, ne_f_files[0]), num_points, device)
        if gal_emb is None:
            continue
        ...
        for f in ne_f_files[1:]:
            emb = _embed(net, os.path.join(d, f), num_points, device)
            if emb is not None:
                probes['NU'][0].append(emb); probes['NU'][1].append(idx)

        for f in fe_files:
            emb = _embed(net, os.path.join(d, f), num_points, device)
            if emb is not None:
                probes['FE'][0].append(emb); probes['FE'][1].append(idx)

        for f in oc_files:
            emb = _embed(net, os.path.join(d, f), num_points, device)
            if emb is not None:
                probes['OC'][0].append(emb); probes['OC'][1].append(idx)
...
def evaluate_facescape(model_path: str, data_root: str,
    ...
    for idx, ident in enumerate(identities):
        ...
        if gal_file is None:
            continue
        gal_emb = _embed(net, os.path.join(d, gal_file), num_points, device)
        if gal_emb is None:
            continue
        gallery_embs.append(gal_emb)
        gallery_ids.append(idx)
        ...
        for f in expr_files:
            emb = _embed(net, os.path.join(d, f), num_points, device)
            if emb is not None:
                fe_embs.append(emb); fe_ids.append(idx)
...
def main():
    ...
    parser.add_argument('--ckpt_umbdb',     default=None,
                        help='UMBDB 체크포인트 (미지정 시 최신 자동 선택)')
    parser.add_argument('--ckpt_facescape', default=None,
                        help='FaceScape 체크포인트 (미지정 시 최신 자동 선택)')
    ...
    UMBDB_CKPT_DIR = os.path.join(BASE, 'checkpoints6_umbdb')
    FS_CKPT_DIR    = os.path.join(BASE, 'checkpoints6_facescape')
